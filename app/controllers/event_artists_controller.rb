class EventArtistsController < ApplicationController
  ...
  private
    ...
    def change_event_date
      ...
      @event.date_from = min(@artist_event.agreed_date_time_and_price.datetime_from, @event.date_from)
      @event.date_to = min(@artist_event.agreed_date_time_and_price.datetime_to, @event.date_to)
      @event.save!
    end
