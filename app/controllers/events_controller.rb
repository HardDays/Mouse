class EventsController < ApplicationController
  ...
  def get_updates
    event_updates = @event.feed_items.where.not(action: :launch_event).select(
      :action, :field, :created_at).order(created_at: :desc).as_json(only: [:action, :field, :created_at]).each {|e| e[:type] = "event"}
  ...
  def search
    ...
    render json: @events.distinct.limit(params[:limit]).offset(params[:offset]).order("events.exact_date_from, events.funding_from"), user: @user, mobile: mobile, status: :ok
  end
